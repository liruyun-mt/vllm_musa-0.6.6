...
dequant_8bit<mt_bfloat16>(int q) {
  ...
  const mt_bfloat162 bias_reg =
      __float2bfloat162_rn(*reinterpret_cast<const float*>(&BIAS));

  // Convert to bfloat162 and apply bias
  typename ScalarType<mt_bfloat16>::FragB frag_b;
  // Note: reverse indexing is intentional because weights are permuted
  frag_b[1] = __hmul2(*reinterpret_cast<const mt_bfloat162*>(&Out1), bias_reg);
  frag_b[0] = __hmul2(*reinterpret_cast<const mt_bfloat162*>(&Out2), bias_reg);
  return frag_b;
}
...
void marlin_mm_f16i4(const void* A, const void* B, void* C, void* s, int prob_m,
                     ...
                     musaStream_t stream, int thread_k, int thread_n, int sms,
                     int max_par) {
...
torch::Tensor fp8_marlin_gemm(torch::Tensor& a, torch::Tensor& b_q_weight,
  ...
  if (a.scalar_type() == at::ScalarType::Half) {
    fp8_marlin::marlin_mm_f16i4<half>(
        ...
        at::cuda::getCurrentMUSAStream(dev), thread_k, thread_n, sms,
        marlin::max_par);
  } else if (a.scalar_type() == at::ScalarType::BFloat16) {
    fp8_marlin::marlin_mm_f16i4<mt_bfloat16>(
        ...
        dev, at::cuda::getCurrentMUSAStream(dev), thread_k, thread_n, sms,
        marlin::max_par);
  } else {
